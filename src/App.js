...
function App() {
  ...
  const fetchRandomPerson = async () => {
    ...
    // destructure values
    const { phone, email, login: { password }, name: { first, last }, dob: { age }, picture: { large: image }, location: { street: { number, name } }} = person
    // es6 feature
    ...
    <main>
      ...
      <div className='block'>
        <div className='container'>
          <img src={(randomPerson && randomPerson.image) || defaultImage} alt='random user' className='user-img'/>
